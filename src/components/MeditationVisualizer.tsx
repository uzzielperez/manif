...
const SVG_SCALE = 0.15;
const SILHOUETTE_X_OFFSET = -40;
const SILHOUETTE_Y_OFFSET = -90;
...
export const MeditationVisualizer: React.FC<MeditationVisualizerProps> = ({ type }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const silhouetteX = cx + SILHOUETTE_X_OFFSET;
      const silhouetteY = cy + SILHOUETTE_Y_OFFSET;
      ...
      if (type === 'aura') {
        ...
        drawNebula(silhouetteX, silhouetteY);
        drawStars();
        drawMontblancAura(silhouetteX, silhouetteY - 10);
        drawSilhouette(silhouetteX, silhouetteY);
      } else if (type === 'chakra') {
        t += 0.05;
